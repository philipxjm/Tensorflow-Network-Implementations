...
def load_stock_data(path, moving_window=32, columns=5, train_test_ratio=4.0):
    ...
    def process_data(data):
        ...
        for idx in range(data.shape[0] - (moving_window + 5)):
            stock_set = np.concatenate((stock_set, np.expand_dims(data[range(idx, idx + (moving_window)), :], axis=0)),
                                       axis=0)
            label_set = np.concatenate((label_set, np.array([data[idx + (moving_window + 5), 3]])))
    ...
    print(stocks_set.shape, labels_set.shape)
